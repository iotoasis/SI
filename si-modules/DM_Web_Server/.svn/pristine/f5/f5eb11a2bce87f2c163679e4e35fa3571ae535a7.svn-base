...
public class HeritFileTool {
    ...
    // Log
    //protected static final Log log = LogFactory.getLog(HeritFileTool.class);
	...
	File f = null;
	try {
	    f = new File(HeritWebUtil.filePathBlackList(targetDirPath));
	...
	try {
	    f = new File(HeritWebUtil.filePathBlackList(baseDirPath));
	    isExist = f.exists();
		...
		f = new File(HeritWebUtil.filePathBlackList(subDirPath));
		...
		if (targetDirPath.equals(dirList[i])) {
		    ...
		    if (new File(HeritWebUtil.filePathBlackList(baseDirPath) + "/" + dirList[i]).isDirectory()) {
			dirFindCnt++;
	...
	String result = "";
	try {
	    f = new File(HeritWebUtil.filePathBlackList(filePath));
	...
	try {
		...
		childFile = new File(HeritWebUtil.filePathBlackList(subDirPath));
	...
	File file = new File(HeritWebUtil.filePathBlackList(filePath));
	String result = "";
	try {
	    /*
	    if(!file.exists()){
	...
	File file = new File(HeritWebUtil.filePathBlackList(dirPath));
	String result = "";
	try {
	    //            if(file.exists()){
	    //            	//log.debug("File Exist..");
	...
	File srcDrctry = new File(HeritWebUtil.filePathBlackList(drctry));
	...
	String result = "";
	File file = new File(HeritWebUtil.filePathBlackList(dirDeletePath));
	if (file.isDirectory()) {
		...
		File f = new File(HeritWebUtil.filePathBlackList(dirDeletePath) + "/" + fileList[i]);
		if (f.isFile()) {
	...
	String result = "";
	File file = new File(HeritWebUtil.filePathBlackList(fileDeletePath));
	if (file.isFile()) {
	...
	try {
	    ...
	    File srcFile = new File(HeritWebUtil.filePathBlackList(file1));
	...
	File file = new File(HeritWebUtil.filePathBlackList(drctry1));
	...
	File file = new File(HeritWebUtil.filePathBlackList(parFile1));
	...
	File file1 = new File(HeritWebUtil.filePathBlackList(cmprFile11));
	File file2 = new File(HeritWebUtil.filePathBlackList(cmprFile22));
	...
	File srcFile = new File(HeritWebUtil.filePathBlackList(src));
	...
	try {
		...
		//log.debug("tar:"+tar);
		File tarFile = new File(HeritWebUtil.filePathBlackList(tar));
	...
	for (int i = 0; i < source.length; i++) {
	    ...
	    File chkFile = new File(HeritWebUtil.filePathBlackList(src));
	    if (!chkFile.exists()) {
		//log.debug("+++ 원본 파일이 존재하지 않습니다.");
		return result;
	    }
	...
	for (int j = 0; j < source.length; j++) {
		...
		// 타겟파일이름 명명
		File chkFile = new File(HeritWebUtil.filePathBlackList(source[j]));
		String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();

		try {
		    // 복사될 target 파일 생성
		    tarTemp = createNewFile(tarTemp);
		    File tarFile = new File(HeritWebUtil.filePathBlackList(tarTemp));
	...
	if (srcFile.exists() && srcFile.isDirectory()) {
		...
		if (result) { //f(result != false){
		    ...
		    // 타겟파일이름 명명
		    File chkFile = new File(HeritWebUtil.filePathBlackList(abspath));
		    String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();
	...
	try {
		...
		tarFile1 = new File(HeritWebUtil.filePathBlackList(tarFile1.getAbsolutePath()) + "/" + srcFile.getName());
	    }
	...
	File f = null;
	try {
	    f = new File(HeritWebUtil.filePathBlackList(originalDirPath));
	    // 원본이 유효해야 진행한다.
	    if (f.exists() && f.isDirectory()) {
		...
		if (targetDirPath1.equals("")) {
		    ...
		    File targetDir = new File(HeritWebUtil.filePathBlackList(targetDirPath1));
		    targetDir.mkdirs();
		    // 디렉토리에 속한 파일들을 복사한다.
		    String[] originalFileList = f.list();
		    if (originalFileList.length > 0) {
			for (int i = 0; i < originalFileList.length; i++) {
			    File subF = new File(HeritWebUtil.filePathBlackList(originalDirPath) + FILE_SEPARATOR + originalFileList[i]);
			    if (subF.isFile()) {
	...
	File f = new File(HeritWebUtil.filePathBlackList(targetDirPath));
	if (!f.exists()) {
	    return 0;
	}
	if (f.isFile()) {
