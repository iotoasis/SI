...
function registerDevice(context, loading) {
	...
	console.log("authId : " + authId + " authPwd : " + authPwd);
	
	if (sn.length == 0) {
		alert("시리얼 번호를 입력하세요.");
		return;
	}
	var context = { "param":{ "deviceModelId":id, "sn":sn, "authId":authId, "authPwd":authPwd}, "handler":regDeviceHandler, "errorHandler": regDeviceErrorHandler} ;
