	...
	var getDeviceModelListSuccessCallback = function(msg, context){
		...
		if(dataList.length != 0){
			$.each(dataList, function(index, data){
				...
				deviceModelListHtml += "	<td><input type='radio' id='checkList' name='checkList' value='" + data.id + "' class='i-checks'></td>";
...
*/		
	...
	var validationCheck = function(oui, manufacturer, modelName, deviceType, profileVer, applyYn){
		...
		if(oui == "" || oui == null){
			errorMessage = "제조사oui는 반드시 입력되어야 하는 필드입니다.\n필드의 입력 상태를 확인해 주십시오.";
			createDialogMessage(errorMessage);
			errorMessage = false;
		}
		if(modelName == "" || modelName == null){
			errorMessage = "모델번호는 반드시 입력되어야 하는 필드입니다.\n필드의 입력 상태를 확인해 주십시오.";
			createDialogMessage(errorMessage);
			errorMessage = false;
		}
		if(applyYn == "" || applyYn == null){
			errorMessage = "적용여부는 반드시 선택되어야 하는 필드입니다.\n필드의 선택 상태를 확인해 주십시오.";
			createDialogMessage(errorMessage);
			errorMessage = false;
		}
	...
	return {
		...
		fnInputValInit: fnInputValInit,
		fnCommFileUpload: fnCommFileUpload
	};
})();