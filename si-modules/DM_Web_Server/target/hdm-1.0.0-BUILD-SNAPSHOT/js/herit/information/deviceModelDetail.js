...
var deviceModel = (function(){
	...
	var addEventHandler = function(){
		...
		$("#modifyDeviceModel").click(function() {
			...
			$("#modifyDeviceModelDialog").dialog("open");
		});
		...
		$("#addOptionData").click(function() {
			$("#actionType").val("I");
			$("#addOptionDataDialog").dialog("open");
		});
		//옵션데이터 삭제
		...
		$("#addErrorCode").click(function() {
			...
			$("#actionType").val("I");
			$("#addErrorCodeDialog").dialog("open");
		});
		//에러코드 삭제
		...
		$("#addNotiCondition").click(function() {
			$("#actionType").val("I");
			$("#addNotiConditionDialog").dialog("open");
		});
		//노티조건 삭제
	...
	var moOptionDataList = function(){
		...
		$("#addOptionDataDialog").dialog("close");
		moProfileList();
...
//		console.log("msg: "+JSON.stringify(msg));
		...
		$("#optionDataListDialog").dialog("open");
	};
	
	var moErrorCodeList = function(){
		$("#addErrorCodeDialog").dialog("close");
		moProfileList();
...
//		console.log("msg: "+JSON.stringify(msg));
		...
		$("#errorCodeListDialog").dialog("open");
	};

	var moNotiConditionList = function(){	
		$("#addNotiConditionDialog").dialog("close");
		moProfileList();
...
//		console.log("msg: "+JSON.stringify(msg));
		...
		if(moNotiConditionList.length != 0){
			...
			$.each(moNotiConditionList, function(idx, moNotiCondition){
				...
				case '0':
					...
					break;
				case '1':
					conditionType = "변경될때마다";
					break;
				case '2':
					conditionType = "노티조건에 부합할때마다";
					break;
				case '3':
					conditionType = "UPDATE주기마다";
					break;
				}
		...
		$("#notiConditionListDialog").dialog("open");
	};
	...
	var createDialog = function(){
		...
		$("#modifyDeviceModelDialog").dialog({
			autoOpen: false,
			width: 575,
	 		height: 410,
	 		modal: true,
		...
		$("#notiConditionListDialog").dialog({
			autoOpen: false,
			width: 675,
	 		height: 410,
	 		modal: true,
		...
		if(moProfileList.length != 0){
			...
			$.each(moProfileList, function(idx, moProfileData){
				...
				case 1:
					...
					break;
				case 2:
					notiType = "<button id='" + moProfileData.id + "' class='notiOptionBtn'>목록(" + moProfileData.moNotiConditionCnt + "건)</button>";
					break;
				case 3:
					notiType = "NO UPDATE";
					break;
				...
				moProfileListHtml += "	<td><button id='" + moProfileData.id + "' class='optionDataBtn'>목록(" + moProfileData.moOptionDataCnt + "건)</button></td>";
				moProfileListHtml += "	<td><button id='" + moProfileData.id + "' class='errorDataBtn'>목록(" + moProfileData.moErrorCodeCnt + "건)</button></td>";
				moProfileListHtml += "	<td>" + notiType + "</td>";
				moProfileListHtml += "	<td>" +
											"<button id='" + moProfileData.id + "' idx='"+idx+"' class='moProfileUpdateBtn'>수정</button>" +
											"<button id='" + moProfileData.id + "' idx='"+idx+"' class='moProfileDeleteBtn'>삭제</button>" +
										"</td>";
				moProfileListHtml += "</tr>";
