...
import net.herit.iot.onem2m.incse.manager.dao.MgmtSoftwareDAO;
import net.herit.iot.onem2m.resource.Reboot;
import net.herit.iot.onem2m.resource.Resource;
import net.herit.iot.onem2m.resource.Software;


public class MgmtSoftwareManager extends AbsManager {
	
	static String ALLOWED_PARENT = "node";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.MGMT_OBJ; 
	
	private Logger log = LoggerFactory.getLogger(MgmtSoftwareManager.class);

	private static final String TAG = MgmtSoftwareManager.class.getName();

	public MgmtSoftwareManager(OneM2mContext context) {
	...
	public OneM2mResponse update(OneM2mRequest reqMessage) throws OneM2MException {
		...
		Software resInDB = (Software)this.getDAO().retrieve(reqMessage.getTo(), RESULT_CONT.ATTRIBUTE);
		if (resInDB == null) {
			throw new OneM2MException(RESPONSE_STATUS.NOT_FOUND, "No resource found:"+reqMessage.getTo());
		}
		
		Software software = (Software)getContentResource(reqMessage, this);
		...
		DMControllerFactory.getController(resInDB, context).executeMgmtObj(resInDB, software);	// updated in 2017-09-16
		
		return updateResource(software, reqMessage, this);
...
//	}
	...
	public DAOInterface getDAO() {
		return new MgmtSoftwareDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return MgmtSoftwareManager.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(Software.class, Software.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(Software.class, Software.SCHEMA_LOCATION);
	}
	
	@Override
	public void updateResource(Resource resource, OneM2mRequest req) throws OneM2MException {
		Software res =  (Software)resource;
