...
public class MgmtCmdManager extends AbsManager {
	...
	public OneM2mResponse update(OneM2mRequest reqMessage) throws OneM2MException {
		...
		OneM2mResponse resMessage = null;			// added in 2017-11-08
		...
		if (execEnable != null && mgmtCmd.isExecEnable()) {
			...
			boolean isTTA = CfgManager.getInstance().getOptionsTTA();
			...
			while (it.hasNext()) {
				...
				if(isTTA) {	// added in 2017-10-31 for TTA
					mgmtCmdDb.addExecInstanceOrSubscription(execInst);
				}
			...
			resMessage = new OneM2mResponse(RESPONSE_STATUS.CHANGED, reqMessage);  // changed to OK to changed in 2017-10-25
			if(isTTA) {
				resMessage.setContentObject(mgmtCmdDb);   // added in 2017-10-30 to support TTA
			} else {
				resMessage.setContentObject(uriList);  // added in 2017-10-30
			}
			...
			//return updateResource(mgmtCmd, reqMessage, this);		// blocked in 2017-11-08
			// added in 2017-11-08 to support TTA as below 'else' paragraph
			updateResource(mgmtCmd, reqMessage, this);
			String to = reqMessage.getTo();
			MgmtCmdDAO dao = (MgmtCmdDAO)this.getDAO();
			MgmtCmd res = (MgmtCmd)dao.retrieve(to, null);
			
			if (res == null) {
				throw new OneM2MException(RESPONSE_STATUS.NOT_FOUND, "Not found");	
			}
			
			resMessage = new OneM2mResponse(RESPONSE_STATUS.CHANGED, reqMessage);
