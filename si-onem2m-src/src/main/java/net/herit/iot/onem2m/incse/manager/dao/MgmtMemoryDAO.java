...
public class MgmtMemoryDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtMemoryDAO.class);

	public MgmtMemoryDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<Memory> jc = (DaoJSONConvertor<Memory>)ConvertorFactory.getDaoJSONConvertor(Memory.class, Memory.SCHEMA_LOCATION);
			return jc.marshal((Memory)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<Memory>)ConvertorFactory.getDaoJSONConvertor(Memory.class, Memory.SCHEMA_LOCATION), rc);
