...
public class MgmtAreaNwkInfoDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtAreaNwkInfoDAO.class);

	public MgmtAreaNwkInfoDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<AreaNwkInfo> jc = (DaoJSONConvertor<AreaNwkInfo>)ConvertorFactory.getDaoJSONConvertor(AreaNwkInfo.class, AreaNwkInfo.SCHEMA_LOCATION);
			return jc.marshal((AreaNwkInfo)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<AreaNwkInfo>)ConvertorFactory.getDaoJSONConvertor(AreaNwkInfo.class, AreaNwkInfo.SCHEMA_LOCATION), rc);
