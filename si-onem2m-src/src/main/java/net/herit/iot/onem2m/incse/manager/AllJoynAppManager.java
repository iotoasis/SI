...
public class AllJoynAppManager extends AbsManager {

	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(AllJoynAppManager.class);

	private static final String TAG = AllJoynAppManager.class.getName();

	public AllJoynAppManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((AllJoynAppDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((AllJoynAppDAO)getDAO()).resourceToJson(orgRes));
	...
	public DAOInterface getDAO() {
		return new AllJoynAppDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return AllJoynApp.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(AllJoynApp.class, AllJoynApp.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(AllJoynApp.class, AllJoynApp.SCHEMA_LOCATION);
	}
