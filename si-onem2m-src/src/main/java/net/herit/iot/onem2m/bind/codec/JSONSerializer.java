...
import net.herit.iot.onem2m.incse.manager.GenericInterworkingOperationInstanceManager;
...
import net.herit.iot.onem2m.resource.GenericInterworkingOperationInstance;
import net.herit.iot.onem2m.resource.GenericInterworkingService;
...
public class JSONSerializer extends AbsSerializer {
	...
	public String serialize(Object content) throws Exception {
		...
		if (content instanceof UriContent || content instanceof UriListContent) {
			...
			} else if (content instanceof SemanticDescriptor) {
				JSONConvertor<SemanticDescriptor> ContInstXC = (JSONConvertor<SemanticDescriptor>)ConvertorFactory.getJSONConvertor(SemanticDescriptor.class, SemanticDescriptor.SCHEMA_LOCATION);
				json = ContInstXC.marshal((SemanticDescriptor)res);	
				
			} else if (content instanceof FlexContainerResource) {
				JSONConvertor<FlexContainerResource> ContInstXC = (JSONConvertor<FlexContainerResource>)ConvertorFactory.getJSONConvertor(FlexContainerResource.class, FlexContainerResource.SCHEMA_LOCATION);
				json = ContInstXC.marshal((FlexContainerResource)res);
			} else if (content instanceof AllJoynApp) {
				JSONConvertor<AllJoynApp> ContInstXC = (JSONConvertor<AllJoynApp>)ConvertorFactory.getJSONConvertor(AllJoynApp.class, AllJoynApp.SCHEMA_LOCATION);
				json = ContInstXC.marshal((AllJoynApp)res);			
			...
			} else if (content instanceof GenericInterworkingOperationInstance) {
				JSONConvertor<GenericInterworkingOperationInstance> ContInstXC = (JSONConvertor<GenericInterworkingOperationInstance>)ConvertorFactory.getJSONConvertor(GenericInterworkingOperationInstance.class, GenericInterworkingOperationInstance.SCHEMA_LOCATION);
				json = ContInstXC.marshal((GenericInterworkingOperationInstance)res);			
			} else {
