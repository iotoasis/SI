...
public class AppEmulatorHttpListener implements HttpServerListener {
	...
	private Logger log = LoggerFactory.getLogger(AppEmulatorHttpListener.class);

	private AEController aeController; 
	private NotiHandlerInterface notiHandler; 
	
	public AppEmulatorHttpListener(String ip, int port, AEController aeController, NotiHandlerInterface notiHandler) {
		try {
...
//			}
			...
			try {
				...
				sendResponse(ctx, HttpResponseCodec.encode(resMessage, httpVersion));
...
//	protected OneM2mResponse processRequest(OneM2mRequest request) throws UnsupportedEncodingException, Exception {
//		
//		// AE는 notification request만 처리함
//		
//		String content = new String(request.getContent(), "UTF-8");
...
//		Notification res = ContInstXC.unmarshal(content);
//		
//		// process request
//		// TBD
//		OneM2mResponse response = new OneM2mResponse(RESPONSE_STATUS.NOT_IMPLEMENTED, request);
//		
//		return response;
//	}
