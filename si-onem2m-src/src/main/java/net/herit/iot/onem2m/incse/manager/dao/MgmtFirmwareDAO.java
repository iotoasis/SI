...
public class MgmtFirmwareDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtFirmwareDAO.class);

	public MgmtFirmwareDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<Firmware> jc = (DaoJSONConvertor<Firmware>)ConvertorFactory.getDaoJSONConvertor(Firmware.class, Firmware.SCHEMA_LOCATION);
			return jc.marshal((Firmware)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<Firmware>)ConvertorFactory.getDaoJSONConvertor(Firmware.class, Firmware.SCHEMA_LOCATION), rc);
