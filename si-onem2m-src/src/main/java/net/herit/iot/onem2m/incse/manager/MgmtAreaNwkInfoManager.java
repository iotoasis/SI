...
import net.herit.iot.onem2m.incse.manager.dao.MgmtAreaNwkInfoDAO;
import net.herit.iot.onem2m.resource.AreaNwkInfo;
import net.herit.iot.onem2m.resource.Resource;


public class MgmtAreaNwkInfoManager extends AbsManager {
	
	static String ALLOWED_PARENT = "node";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.MGMT_OBJ; 
	
	private Logger log = LoggerFactory.getLogger(MgmtAreaNwkInfoManager.class);

	private static final String TAG = MgmtAreaNwkInfoManager.class.getName();

	public MgmtAreaNwkInfoManager(OneM2mContext context) {
...
//	}
	...
	public DAOInterface getDAO() {
		return new MgmtAreaNwkInfoDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return MgmtAreaNwkInfoManager.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(AreaNwkInfo.class, AreaNwkInfo.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(AreaNwkInfo.class, AreaNwkInfo.SCHEMA_LOCATION);
	}
	
	@Override
	public void updateResource(Resource resource, OneM2mRequest req) throws OneM2MException {
		AreaNwkInfo res =  (AreaNwkInfo)resource;
