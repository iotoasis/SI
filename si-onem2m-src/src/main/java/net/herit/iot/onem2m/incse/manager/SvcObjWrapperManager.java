...
public class SvcObjWrapperManager extends AbsManager {
	
	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(SvcObjWrapperManager.class);

	private static final String TAG = SvcObjWrapperManager.class.getName();

	public SvcObjWrapperManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((SvcObjWrapperDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((SvcObjWrapperDAO)getDAO()).resourceToJson(orgRes));
		NotificationController.getInstance().notify(parentRes, reqDoc, reqRes, orgDoc, orgRes, op, this);
	}
	
	@Override
	public DAOInterface getDAO() {
		return new SvcObjWrapperDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return SvcObjWrapper.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(SvcObjWrapper.class, SvcObjWrapper.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(SvcObjWrapper.class, SvcObjWrapper.SCHEMA_LOCATION);
	}
