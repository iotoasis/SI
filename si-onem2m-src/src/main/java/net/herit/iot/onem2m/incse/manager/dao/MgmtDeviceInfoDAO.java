...
public class MgmtDeviceInfoDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtDeviceInfoDAO.class);

	public MgmtDeviceInfoDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<DeviceInfo> jc = (DaoJSONConvertor<DeviceInfo>)ConvertorFactory.getDaoJSONConvertor(DeviceInfo.class, DeviceInfo.SCHEMA_LOCATION);
			return jc.marshal((DeviceInfo)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<DeviceInfo>)ConvertorFactory.getDaoJSONConvertor(DeviceInfo.class, DeviceInfo.SCHEMA_LOCATION), rc);
