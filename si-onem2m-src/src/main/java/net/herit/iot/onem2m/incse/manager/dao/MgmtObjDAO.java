...
public class MgmtObjDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtObjDAO.class);

	public MgmtObjDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<MgmtResource> jc = (DaoJSONConvertor<MgmtResource>)ConvertorFactory.getDaoJSONConvertor(MgmtResource.class, MgmtResource.SCHEMA_LOCATION);
			return jc.marshal((MgmtResource)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<MgmtResource>)ConvertorFactory.getDaoJSONConvertor(MgmtResource.class, MgmtResource.SCHEMA_LOCATION), rc);
