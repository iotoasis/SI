...
//		if(pollingChannels.size() == 0) return;
	private void sendAnnounceWithPollingChannel(OneM2mRequest reqMessage, Resource resAnnc, String pcu, OPERATION op) {
		...
		//content.getAny().add(resAnnc);	updated in 2016-10-17 for CDT-2.7.0 as below
		content.getAnyOrAny().add(resAnnc);
		...
		if(reqMessage.getFilterCriteria() != null) {
			...
			filterC.setLevel(reqMessage.getFilterCriteria().getLevel());
			filterC.setOffset(reqMessage.getFilterCriteria().getOffset());
