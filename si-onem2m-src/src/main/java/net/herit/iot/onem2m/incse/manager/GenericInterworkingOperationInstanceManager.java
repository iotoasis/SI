...
import net.herit.iot.onem2m.incse.manager.dao.GenericInterworkingOperationInstanceDAO;
...
public class GenericInterworkingOperationInstanceManager extends AbsManager {

	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(GenericInterworkingServiceManager.class);

	private static final String TAG = GenericInterworkingServiceManager.class.getName();

	public GenericInterworkingOperationInstanceManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((GenericInterworkingOperationInstanceDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((GenericInterworkingOperationInstanceDAO)getDAO()).resourceToJson(orgRes));
	...
	public DAOInterface getDAO() {
		return new GenericInterworkingOperationInstanceDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return GenericInterworkingOperationInstance.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(GenericInterworkingOperationInstance.class, GenericInterworkingOperationInstance.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(GenericInterworkingOperationInstance.class, GenericInterworkingOperationInstance.SCHEMA_LOCATION);
	}
