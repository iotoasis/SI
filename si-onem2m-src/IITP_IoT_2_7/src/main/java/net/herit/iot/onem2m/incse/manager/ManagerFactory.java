...
//			break;
	...
	static public ManagerInterface create(OneM2mRequest reqMessage, OneM2mContext context) throws OneM2MException {
		...
		if(targetAddress.startsWith("-")) {
			targetAddress = targetAddress.replaceFirst("-", CfgManager.getInstance().getCSEBaseCid() + "/" + CfgManager.getInstance().getCSEBaseName());
			reqMessage.setTo(targetAddress);
		}
		
		// added in 2017-04-21 to support unstructured-CSE-relative Resource ID
		if(!targetAddress.startsWith("/" + CfgManager.getInstance().getCSEBaseCid())) {	
			String[] arrUriItems = targetAddress.split("/");
