...
import net.herit.iot.onem2m.incse.manager.dao.AllJoynAppDAO;
...
public class AllJoynSvcObjectManager extends AbsManager {
	
	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(AllJoynSvcObjectManager.class);

	private static final String TAG = AllJoynSvcObjectManager.class.getName();

	public AllJoynSvcObjectManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((AllJoynSvcObjectDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((AllJoynSvcObjectDAO)getDAO()).resourceToJson(orgRes));
		NotificationController.getInstance().notify(parentRes, reqDoc, reqRes, orgDoc, orgRes, op, this);
	}
	
	@Override
	public DAOInterface getDAO() {
		return new AllJoynSvcObjectDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return AllJoynSvcObject.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(AllJoynSvcObject.class, AllJoynSvcObject.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(AllJoynSvcObject.class, AllJoynSvcObject.SCHEMA_LOCATION);
	}
