...
public class ExecInstanceManager extends AbsManager {
	...
	//static String ALLOWED_PARENT = "CSEBase";		changed to mgmtCmd in CDT-2.7.0
	static String ALLOWED_PARENT = "mgmtCmd";
	...
	public ExecInstance createResource(MgmtCmd mgmtCmd, String targetId) throws OneM2MException {
		...
		execInst.setResourceName(execInstId);   // added in 2017-11-08
	...
	public OneM2mResponse update(OneM2mRequest reqMessage) throws OneM2MException {
		...
		// add in 2017-10-17 to support onem2m Autentication Testing
		
		ExecInstance res = (ExecInstance)getContentResource(reqMessage, this);

		if(res.isExecDisable()) {
			update(reqMessage, this);		
			
			
			String to = reqMessage.getTo();
			ExecInstanceDAO dao = (ExecInstanceDAO)this.getDAO();
			
			ExecInstance eiRes = (ExecInstance)dao.retrieve(to, null);
			...
			setResponseContentObjectForRetrieve(resMessage, reqMessage.getResultContentEnum(), eiRes, this);		
	...
	public OneM2mResponse delete(OneM2mRequest reqMessage) throws OneM2MException {
		...
		return delete(reqMessage, this);			// updated in 2017-10-17 to support execInstance delete
