...
//			long expireAfter = SIMPLE_DATE_FORMAT.parse(evt.getExpireAfter()).getTime();
	...
	private void sendNotifyWithPollingChannel(OneM2mRequest reqMessage, String pcu) {
		...
		//content.addAny(reqMessage.getContentObject());	updated in 2016-10-17 for CDT-2.7.0 as below
		content.addAnyOrAny(reqMessage.getContentObject()); 
	...
	private void sendNotify(OneM2mRequest reqMessage, String to, ResourceDAO dao) throws Exception {
		...
		for(String uri : targetURI) {
			try {
				...
				//added in 2017-05-16, updated to support "/~/herit-in/herit-cse" in uri
				String uriPath = _uri_.getPath();
				if(uriPath.startsWith("/~")) {
					uriPath = uriPath.substring(2);
				}
			
				if(_uri_.getPath().length() > 1) {
					//reqMessage.setTo(uri.substring(uri.indexOf(_uri_.getPath())));		// blocked in 2017-05-16
					reqMessage.setTo(uri.substring(uri.indexOf(uriPath)));
				} else {
					reqMessage.setTo("/");
				}
				...
				if(!_uri_.getScheme().equals("mqtt") &&_uri_.getHost().equals(CfgManager.getInstance().getHostname())) {   // updated in 2017-11-07 to support MQTT Notify
					//log.debug("pollingChannel={}, pcu={}", uri, _uri_.getPath());		// blocked in 2017-05-16
					log.debug("pollingChannel={}, pcu={}", uri, uriPath);
					//if(dao.isVaildPollingChannelUri(_uri_.getPath())) {				// blocked in 2017-05-16
					if(dao.isVaildPollingChannelUri(uriPath)) {
						//sendNotifyWithPollingChannel(reqMessage, _uri_.getPath());	// blocked in 2017-05-16
						sendNotifyWithPollingChannel(reqMessage, uriPath);
					} else {
						log.warn("PollingChannel is not available..{}", uri);
					}
				} else {
					if(_uri_.getScheme().equals("mqtt")) {		// added in 2017-11-07 to support for target token processing.
						String credential = "";
						if(to.startsWith("/")) {
							credential = to;
						} else {
						...
						reqMessage.setCredentialID(credential);
					}
...
//				noti.setSubscriptionReference(orgDoc.getString(Naming.URI_KEY));
			...
			for (Subscription sub : subscriptions) {
				...
				if(evt != null) {
					try {
						...
						if(evt.getNotificationEventType() != null) {
							...
							//if(sub.getParentID() == resourceId) {   //blocked in 2017-10-26
							if(sub.getParentID() != null && sub.getParentID().equals(resourceId)) {  // added in 2017-10-26 		
					...
					// added in 2017-10-25 to process default notification event type..
					if(!sub.getParentID().equals(resourceId) || op != OPERATION.UPDATE) {
						continue;
					} 
					//System.out.println("[Notify]#################### Parent Resource of subscription is updated...");
				}
				...
				noti.setSubscriptionReference(sub.getUri());	// 모다정보통신 대응, 2016.09.28, by brianmoon
