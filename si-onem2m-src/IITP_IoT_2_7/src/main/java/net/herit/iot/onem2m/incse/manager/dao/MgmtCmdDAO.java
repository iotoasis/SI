...
public class MgmtCmdDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtCmdDAO.class);

	public MgmtCmdDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<MgmtCmd> jc = (DaoJSONConvertor<MgmtCmd>)ConvertorFactory.getDaoJSONConvertor(MgmtCmd.class, MgmtCmd.SCHEMA_LOCATION);
			return jc.marshal((MgmtCmd)res);
	...
	public void create(Resource resource) throws OneM2MException {
		// added in 2017-10-25
		if(((MgmtCmd)resource).getExpirationTime() != null) {
			((MgmtCmd)resource).setExpirationTime(CfgManager.getInstance().getDefaultExpirationTime());
		}
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<MgmtCmd>)ConvertorFactory.getDaoJSONConvertor(MgmtCmd.class, MgmtCmd.SCHEMA_LOCATION), rc);
