...
import net.herit.iot.onem2m.incse.manager.dao.AllJoynInterfaceDAO;
import net.herit.iot.onem2m.incse.manager.dao.AllJoynSvcObjectDAO;
...
public class AllJoynInterfaceManager extends AbsManager {
	
	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(AllJoynInterfaceManager.class);

	private static final String TAG = AllJoynInterfaceManager.class.getName();

	public AllJoynInterfaceManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((AllJoynInterfaceDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((AllJoynInterfaceDAO)getDAO()).resourceToJson(orgRes));
		NotificationController.getInstance().notify(parentRes, reqDoc, reqRes, orgDoc, orgRes, op, this);
	}
	
	@Override
	public DAOInterface getDAO() {
		return new AllJoynInterfaceDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return AllJoynInterface.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(AllJoynInterface.class, AllJoynInterface.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(AllJoynInterface.class, AllJoynInterface.SCHEMA_LOCATION);
	}
