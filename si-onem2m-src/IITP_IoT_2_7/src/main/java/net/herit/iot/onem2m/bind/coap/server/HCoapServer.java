...
//		add(new CoapResource(resourceId).add(new HCoapResource(resourceName)));
	...
	public HCoapServer(int port, int sec_port) throws SocketException {		// added in 2017-11-14 for construcor to suppport cse-relative unstructured addressing 
		this.port = port;
		this.sec_port = sec_port;
	}
	
	public void addEndPointsbyResId(ArrayList<String> resIds, String cseBase) throws SocketException {	// added in 2017-11-14 to add endpoints for resource id in mongodb
		add(new HCoapResource(cseBase));
		add(new HCoapResource("~"));	// for SP-relative Resource ID
		add(new HCoapResource("_"));	// Absolute Resource ID
		
		for(String resId : resIds) {
			add(new HCoapResource(resId));
		}
	...
	// added in 2017-11-14
	public HCoapServer(CoapServerListener listener, int port, int sec_port)
			throws SocketException {
