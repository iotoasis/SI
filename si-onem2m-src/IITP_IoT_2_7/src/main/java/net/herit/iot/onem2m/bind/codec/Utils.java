...
public class Utils {
	...
	private static String toParamCheck(String url) {
		...
		if(url.startsWith("/~/")) {	      // for http (structured SP-Relative)
			...
			//url = "/" + url.substring(2);		blocked in 2017-08-21
			String tmpUrl = url.substring(3);						// added in 2017-08-21 to support absolute addressing
...
//		}
	...
	private final static String HTTP_QUERY_OFFSET 		= "ofst";
	private final static String HTTP_QUERY_LEVEL 		= "lvl";
	...
	public static void decodeQueryString(OneM2mRequest reqMessage, HashMap<String, Object> queries) throws Exception {
		...
		// added in 2016-11-24 to support 2.7
		value = queries.get(HTTP_QUERY_LEVEL);
		if (null != value) {
			if (!(value instanceof String)) throw new OneM2MException(RESPONSE_STATUS.INVALID_ARGUMENTS, HTTP_QUERY_LEVEL+ " cannot be list.");
			if(filterCriteria == null) filterCriteria = new FilterCriteria();
			filterCriteria.setLevel(Integer.parseInt((String)value));
		}
		
		// added in 2016-11-24 to support 2.7
		value = queries.get(HTTP_QUERY_OFFSET);
		if (null != value) {
			if (!(value instanceof String)) throw new OneM2MException(RESPONSE_STATUS.INVALID_ARGUMENTS, HTTP_QUERY_OFFSET+ " cannot be list.");
			if(filterCriteria == null) filterCriteria = new FilterCriteria();
			filterCriteria.setOffset(Integer.parseInt((String)value));
		}
...
//					filterCriteria.putAttribute(split[0], split[1]);
		...
		if(filterCriteria != null) {
			Iterator<String> keys = queries.keySet().iterator();
			while(keys.hasNext()) {
