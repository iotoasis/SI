...
public class MgmtSoftwareDAO extends ResourceDAO implements DAOInterface {
	
	private Logger log = LoggerFactory.getLogger(MgmtSoftwareDAO.class);

	public MgmtSoftwareDAO(OneM2mContext context) {
		super(context);
	}
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			DaoJSONConvertor<Software> jc = (DaoJSONConvertor<Software>)ConvertorFactory.getDaoJSONConvertor(Software.class, Software.SCHEMA_LOCATION);
			return jc.marshal((Software)res);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(DaoJSONConvertor<Software>)ConvertorFactory.getDaoJSONConvertor(Software.class, Software.SCHEMA_LOCATION), rc);
