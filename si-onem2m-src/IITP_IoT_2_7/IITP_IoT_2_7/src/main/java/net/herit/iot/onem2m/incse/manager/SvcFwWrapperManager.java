...
import net.herit.iot.onem2m.incse.manager.dao.SvcFwWrapperDAO;
import net.herit.iot.onem2m.incse.manager.dao.SvcObjWrapperDAO;
...
public class SvcFwWrapperManager extends AbsManager {
	
	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(SvcFwWrapperManager.class);

	private static final String TAG = SvcFwWrapperManager.class.getName();

	public SvcFwWrapperManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((SvcFwWrapperDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((SvcFwWrapperDAO)getDAO()).resourceToJson(orgRes));
		NotificationController.getInstance().notify(parentRes, reqDoc, reqRes, orgDoc, orgRes, op, this);
	}
	
	@Override
	public DAOInterface getDAO() {
		return new SvcFwWrapperDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return SvcFwWrapper.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(SvcFwWrapper.class, SvcFwWrapper.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(SvcFwWrapper.class, SvcFwWrapper.SCHEMA_LOCATION);
	}
