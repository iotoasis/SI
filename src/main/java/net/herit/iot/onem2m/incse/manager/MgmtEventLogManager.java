...
import net.herit.iot.onem2m.incse.manager.dao.MgmtEventLogDAO;
...
public class MgmtEventLogManager extends AbsManager {
	
	static String ALLOWED_PARENT = "node";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.MGMT_OBJ; 
	
	private Logger log = LoggerFactory.getLogger(MgmtEventLogManager.class);

	private static final String TAG = MgmtEventLogManager.class.getName();

	public MgmtEventLogManager(OneM2mContext context) {
	...
	public OneM2mResponse retrieve(OneM2mRequest reqMessage) throws OneM2MException {
		...
		EventLog evtLog = (EventLog)this.retrieveResource(reqMessage, this);
		
		//evtLog = (EventLog)DMControllerFactory.getController(evtLog).getStatus(evtLog);
		evtLog = (EventLog)DMControllerFactory.getController(evtLog, context).getStatus(evtLog);	// updated in 2017-08-23
		
		OneM2mResponse resMessage = new OneM2mResponse(RESPONSE_STATUS.OK, reqMessage);
		setResponseContentObjectForRetrieve(resMessage, reqMessage.getResultContentEnum(), evtLog, this);
	...
	public OneM2mResponse update(OneM2mRequest reqMessage) throws OneM2MException {
		...
		EventLog resInDB = (EventLog)this.getDAO().retrieve(reqMessage.getTo(), RESULT_CONT.ATTRIBUTE);
		if (resInDB == null) {
			throw new OneM2MException(RESPONSE_STATUS.NOT_FOUND, "No resource found:"+reqMessage.getTo());
		}
		
		EventLog evtLog = (EventLog)getContentResource(reqMessage, this);
		
		//DMControllerFactory.getController(evtLog).executeMgmtObj(resInDB, evtLog);
		//DMControllerFactory.getController(evtLog, context).executeMgmtObj(resInDB, evtLog);	// updated in 2017-08-23
		evtLog = DMControllerFactory.getController(resInDB, context).executeMgmtObj(resInDB, evtLog);	// updated in 2017-09-16
		
		return updateResource(evtLog, reqMessage, this);
...
//	}
	...
	public DAOInterface getDAO() {
		return new MgmtEventLogDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return MgmtEventLogManager.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(EventLog.class, EventLog.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(EventLog.class, EventLog.SCHEMA_LOCATION);
	}
	
	@Override
	public void updateResource(Resource resource, OneM2mRequest req) throws OneM2MException {
		EventLog res =  (EventLog)resource;
