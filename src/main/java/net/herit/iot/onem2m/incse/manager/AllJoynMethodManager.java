...
public class AllJoynMethodManager extends AbsManager {
	
	static String ALLOWED_PARENT = "CSEBase,AE,remoteCSE,container";  
	static RESOURCE_TYPE RES_TYPE = RESOURCE_TYPE.FLEXCONTAINER; 
	
	private Logger log = LoggerFactory.getLogger(AllJoynMethodManager.class);

	private static final String TAG = AllJoynMethodManager.class.getName();

	public AllJoynMethodManager(OneM2mContext context) {
	...
	public void notification(Resource parentRes, Resource reqRes, Resource orgRes, OPERATION op) throws OneM2MException {
		Document reqDoc = reqRes == null? null : Document.parse( ((AllJoynMethodDAO)getDAO()).resourceToJson(reqRes));
		Document orgDoc = orgRes == null? null : Document.parse( ((AllJoynMethodDAO)getDAO()).resourceToJson(orgRes));
		NotificationController.getInstance().notify(parentRes, reqDoc, reqRes, orgDoc, orgRes, op, this);
	}
	
	@Override
	public DAOInterface getDAO() {
		return new AllJoynMethodDAO(context);
	}

	@Override
	public Class<?> getResourceClass() {
		return AllJoynMethod.class;
	}

	@Override
	public XMLConvertor<?> getXMLConveter() throws OneM2MException {
		return ConvertorFactory.getXMLConvertor(AllJoynMethod.class, AllJoynMethod.SCHEMA_LOCATION);
	}

	@Override
	public JSONConvertor<?> getJSONConveter() throws OneM2MException {
		return ConvertorFactory.getJSONConvertor(AllJoynMethod.class, AllJoynMethod.SCHEMA_LOCATION);
	}
