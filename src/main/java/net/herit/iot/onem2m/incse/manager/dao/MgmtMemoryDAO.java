...
public class MgmtMemoryDAO extends ResourceDAO implements DAOInterface {
	...
	public String resourceToJson(Resource res) throws OneM2MException {
		try {
			...
			JSONConvertor<Memory> jc = (JSONConvertor<Memory>)ConvertorFactory.getJSONConvertor(Memory.class, Memory.SCHEMA_LOCATION);
	...
	public Resource retrieve(String id, RESULT_CONT rc) throws OneM2MException {
		...
		return retrieve(OneM2mUtil.isUri(id) ? URI_KEY : RESID_KEY, id, 
				(JSONConvertor<Memory>)ConvertorFactory.getJSONConvertor(Memory.class, Memory.SCHEMA_LOCATION), rc);
