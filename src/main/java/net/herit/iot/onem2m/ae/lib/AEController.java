...
public class AEController {
	...
	/**
	  ...
	  * @param maxCvtCnt - Convertor 풀내에 관리될 최대 Convertor 수
	 * @throws OneM2MException 
	  * 
	  */
	public AEController(String cseAddr, String cseId, String csebaseName, 
						CONTENT_TYPE contentType, AuthControllerInterface authController, int maxCvtCnt) throws OneM2MException {
		...
		ConvertorFactory.initialize(maxCvtCnt);
	...
	/**
	  * Notification XML Convertor를 초기화 한다.
	  * 
	  * @param contentType - 초기화될 Convertor의 contentType, CONTENT_TYPE.XML 또는 CONTENT_TPYE.JSON 입력. 
	  * @param resType - 초기화될 Convertor의 ResourceType
	  */
	public void doInitializeConvertor(CONTENT_TYPE contentType, RESOURCE_TYPE resType) throws Exception {
		...
		if (contentType == CONTENT_TYPE.XML) {
			...
			case CONTENT_INST:
				...
				break;
			default:
				throw new OneM2MException(RESPONSE_STATUS.INVALID_ARGUMENTS, "Not supported resource type:"+resType.Name());
			} 
		} else if (contentType == CONTENT_TYPE.JSON){
			JSONConvertor<?> jsonCvt; // NOPMD by inseu_000 on 16. 1. 19 오후 4:37
			switch (resType) {
			case NOTIFICATION:
				jsonCvt = ConvertorFactory.getJSONConvertor(Notification.class, Notification.SCHEMA_LOCATION);
				break;
			case ACCESS_CTRL_POLICY:
				jsonCvt = ConvertorFactory.getJSONConvertor(AccessControlPolicy.class, AccessControlPolicy.SCHEMA_LOCATION);
				break;
			case AE:
			...
			throw new OneM2MException(RESPONSE_STATUS.INVALID_ARGUMENTS, "Not supported content type:"+contentType.Name());
		}		
	}
	...
	/**
	  ...
	  * @param cntr - 생성될 container 정보
	 * @param acpIds 
	  * @return 생성된 container정보 오브젝트  
	  */
	public Container doCreateContainer(String aeUri, String from, String containerName, List<String> acpIds, Container cntr,
										boolean retrieveBeforeCreate) throws Exception {
				
		// 이미 등록된 경우 Container정보 retrieve
		if (retrieveBeforeCreate) {
			try {
				Container cntr1 = (Container)this.retrieveContainer(aeUri +"/"+containerName, from);
				return cntr1;
			} catch (OneM2MException e) {
				if (e.getResponseStatusCode() != RESPONSE_STATUS.NOT_FOUND) {
					throw e;
		...
		if (acpIds != null) {
			...
			while (it.hasNext()) {
				cntr.addAccessControlPolicyIDs(it.next());			
			}
		}
		
		return createContainer(aeUri, from, cntr, containerName);
...
//	}
	...
	public HttpBasicResponse doProcessRequest(HttpBasicRequest request, NotiHandlerInterface notiHandler) throws Exception {
		...
		OneM2mRequest reqMessage = RequestCodec.decode(request.getMethod(), request.getUri(), request.getHeaders(), 
				request.getHost(), request.getContent());
		
		OneM2mResponse resMessage = doProcessRequest(reqMessage, notiHandler);
		
		HttpBasicResponse response = ResponseCodec.encodeToBasic(resMessage, HttpVersion.HTTP_1_1);
	...
	public OneM2mResponse doProcessRequest(OneM2mRequest reqMessage, NotiHandlerInterface notiHandler) throws Exception {
		...
		if (ne == null) {
			...
			res.setContent(new String("NotificationEvent should be provided!").getBytes()); // NOPMD by inseu_000 on 16. 1. 19 오후 4:35
			return res;
		}
	...
	private String cseAddr;
	private CONTENT_TYPE contentType;
	
	private AE ae = null;
	private String csebaseUri;
	private String cseId;

	private AuthControllerInterface authController = null;
	
	
	private Resource convertToResource(RESOURCE_TYPE resType, OneM2mResponse res) throws OneM2MException {
	...
	private Resource convertToResource(OneM2mResponse res) throws OneM2MException {
	...
	private Resource convertToResource(RESOURCE_TYPE resType, String content) throws OneM2MException {
		try {
			if (contentType == CONTENT_TYPE.RES_XML) {
				...
				case CONTENT_INST:
					//xmlCvt = ConvertorFactory.getXMLConvertor(ContentInstance.class, ContentInstance.SCHEMA_LOCATION);
					xmlCvt = ConvertorFactory.getXMLConvertorCI();
					break;
				case SUBSCRIPTION:
				...
				case ACCESS_CTRL_POLICY:
					...
					break;
				case GROUP:
					xmlCvt = ConvertorFactory.getXMLConvertor(Group.class, Group.SCHEMA_LOCATION);
					break;
				case AE_ANNC:
					xmlCvt = ConvertorFactory.getXMLConvertor(AEAnnc.class, AEAnnc.SCHEMA_LOCATION);
					break;
				case CONTAINER_ANNC:
					xmlCvt = ConvertorFactory.getXMLConvertor(ContainerAnnc.class, ContainerAnnc.SCHEMA_LOCATION);
					break;
				case CONTENT_INST_ANNC:
					xmlCvt = ConvertorFactory.getXMLConvertor(ContentInstanceAnnc.class, ContentInstanceAnnc.SCHEMA_LOCATION);
					break;
				}
	...
	private AggregatedResponse convertToAggregatedResponse(String content) throws Exception {
		if (contentType == CONTENT_TYPE.RES_XML) {
			XMLConvertor<?> xmlCvt = ConvertorFactory.getXMLConvertor(AggregatedResponse.class, AggregatedResponse.SCHEMA_LOCATION);
			if (xmlCvt == null) {
				return null;
			} else	{
				return (AggregatedResponse) xmlCvt.unmarshal(content);
			}
		}else {
			JSONConvertor<?> jsonCvt = ConvertorFactory.getJSONConvertor(Notification.class, AggregatedResponse.SCHEMA_LOCATION);
			if (jsonCvt == null) {
				return null;
			} else {
				return (AggregatedResponse)jsonCvt.unmarshal(content);
			}
		}
	}
	
	private Notification convertToNotification(String content) throws Exception {
		if (contentType == CONTENT_TYPE.RES_XML) {
	...
	private UriListContent convertToUriList(String content) throws Exception {
		if (contentType == CONTENT_TYPE.RES_XML) {
	...
	public AE retrieveAE(String resUri, String from) throws OneM2MException {
	...
	public AEAnnc retrieveAEAnnc(String resUri, String from) throws OneM2MException {

		OneM2mResponse res = retrieve(resUri, from);
		AEAnnc ae = (AEAnnc)convertToResource(RESOURCE_TYPE.AE_ANNC, res);
		if (ae.getUri() == null) {
			ae.setUri(resUri);
		}
		return ae;
	...
	private AE updateAE(String resUri, AE ae, String from) throws OneM2MException {
	...
	public AccessControlPolicy retrieveACP(String resUri, String from) throws OneM2MException {
	...
	public Container retrieveContainer(String resUri, String from) throws OneM2MException {
	...
	public ContainerAnnc retrieveContainerAnnc(String resUri, String from) throws OneM2MException {

		OneM2mResponse res = retrieve(resUri, from);
		ContainerAnnc containerA = (ContainerAnnc)convertToResource(RESOURCE_TYPE.CONTAINER_ANNC, res);
		if (containerA.getUri() == null) {
			containerA.setUri(resUri);
		}
		return containerA;
	}
	
	public AggregatedResponse retrieveFanoutContainer(String resUri, String from) throws Exception, OneM2MException {

		OneM2mResponse res = retrieve(resUri, from);
		AggregatedResponse agr = convertToAggregatedResponse(new String(res.getContent(), "UTF-8"));
//		if (container.getUri() == null) {
//			container.setUri(resUri);
//		}
		return agr;
	...
	public Subscription retrieveSubscription(String resUri, String from) throws OneM2MException {
	...
	public ContentInstance retrieveContentInstance(String resUri, String from) throws OneM2MException {
	...
	public ContentInstanceAnnc retrieveContentInstanceAnnc(String resUri, String from) throws OneM2MException {

		OneM2mResponse res = retrieve(resUri, from);
		ContentInstanceAnnc ciA = (ContentInstanceAnnc)convertToResource(RESOURCE_TYPE.CONTENT_INST_ANNC, res);
		if (ciA.getUri() == null) {
			ciA.setUri(resUri);
		}
		return ciA;
	...
	public Group retrieveGroup(String resUri, String from) throws OneM2MException {

		OneM2mResponse res = retrieve(resUri, from);
		Group group = (Group)convertToResource(RESOURCE_TYPE.GROUP, res);
...
//	}
	...
	public OneM2mResponse update(String resUri, String from, Resource resource) throws OneM2MException {
		OneM2mRequest request = new OneM2mRequest();
	...
	private OneM2mResponse retrieve(String resUri, String from) throws OneM2MException {
		OneM2mRequest request = new OneM2mRequest();
		request.setOperation(OPERATION.RETRIEVE);
		request.setTo(resUri);		// "/casebase/SAE_1"
		request.setFrom(from);	// "/SAE_1"
	...
	public Subscription createSubscription(String targetUri, String from, String subsName, Subscription subs) throws OneM2MException {
	...
	public AE createAE(String parentUri, String from, AE ae, String name) throws OneM2MException {
	...
	public AccessControlPolicy createACP(String parentUri, String from, AccessControlPolicy acp, String name) throws OneM2MException {
...
//	}
	...
	public Container createContainer(String parentUri, String from, Container container, String name) throws OneM2MException {
		OneM2mRequest request = new OneM2mRequest();
		request.setOperation(OPERATION.CREATE);
		request.setTo(parentUri);
		request.setFrom(from);
	...
	public AggregatedResponse createFanoutContainer(String parentUri, String from, Container container, String name) throws Exception, OneM2MException {
		OneM2mRequest request = new OneM2mRequest();
		request.setOperation(OPERATION.CREATE);
		request.setTo(parentUri);
		request.setFrom(from);
	...
	public ContentInstance createContentInstance(String parentUri, String from, ContentInstance ci, String ciName) throws OneM2MException {
	...
	public Group createGroup(String parentUri, String from, Group group, String groupName) throws OneM2MException {
		...
		request.setFrom(from);
		request.setRequestIdentifier(Utils.createRequestId());
		request.setResourceType(RESOURCE_TYPE.GROUP);
		request.setContentType(CONTENT_TYPE.RES_XML);
		request.setResultContent(RESULT_CONT.ATTRIBUTE);
		request.setContentObject(group);
		if (groupName != null) {
		...
		if (res.getResponseStatusCodeEnum() == RESPONSE_STATUS.CREATED) {
			Group g = (Group)convertToResource(RESOURCE_TYPE.GROUP, res);
			g.setUri(parentUri +"/"+ g.getResourceName());
			return g;
		} else {
			log.error("createContainer failed.");
			throw new OneM2MException(res.getResponseStatusCodeEnum(), "Faile to createContainer RES:null"+res.toString());
	...
	public Request createRequest(String parentUri, String from, Request reqVO, String reqVOName) throws OneM2MException {
		...
		request.setFrom(from);
		request.setRequestIdentifier(Utils.createRequestId());
		request.setResourceType(RESOURCE_TYPE.REQUEST);
		request.setContentType(CONTENT_TYPE.RES_XML);
		request.setResultContent(RESULT_CONT.ATTRIBUTE);
		request.setContentObject(reqVO);
		if (reqVOName != null) {
			request.setName(reqVOName);
		}
		...
		if (res.getResponseStatusCodeEnum() == RESPONSE_STATUS.CREATED) {
			Request req = (Request)convertToResource(RESOURCE_TYPE.REQUEST, res);
			req.setUri(parentUri +"/"+ req.getResourceName());
			return req;
		} else {
			log.error("createContainer failed.");
			throw new OneM2MException(res.getResponseStatusCodeEnum(), "Faile to createContainer RES:null"+res.toString());
	...
	public OneM2mResponse deleteResource(String uri, String from) throws OneM2MException {
		...
		if (res.getResponseStatusCodeEnum() == RESPONSE_STATUS.DELETED) {
			log.debug(res.getResponseStatusCodeEnum().toString());
