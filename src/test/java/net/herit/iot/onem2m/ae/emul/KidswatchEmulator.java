...
public class KidswatchEmulator {
	...
	public static final String CNT_KIDSWATCH_LOCATION = "cnt-location";
	public static final String CNT_KIDSWATCH_RINGERMODE = "cnt-ringtone";
	...
	private Container locationReqCnt;
	private Container ringtoneCnt;
	private Container ringtoneReqCnt;
	private Subscription locationReqCntSubs;
	private Subscription ringtoneReqCntSubs;
	...
	public void start() throws Exception {
		...
		try {
			...
			// 컨테이너 생성
			locationCnt = aeController.doCreateContainer(ae.getUri(), ae.getAEID(), CNT_KIDSWATCH_LOCATION, acpIds, true);
			locationReqCnt = aeController.doCreateContainer(ae.getUri(), ae.getAEID(), CNT_KIDSWATCH_LOCATION_REQ, acpIds, true);
			ringtoneCnt = aeController.doCreateContainer(ae.getUri(), ae.getAEID(), CNT_KIDSWATCH_RINGERMODE, acpIds, true);
			ringtoneReqCnt = aeController.doCreateContainer(ae.getUri(), ae.getAEID(), CNT_KIDSWATCH_RINGERMODE_REQ, acpIds, true);
			
			// 컨테이너에 대한 구독 요청
			locationReqCntSubs = aeController.doCreateSubscription(locationCnt.getUri(), this.aeId, 
					"sub-"+this.ae.getResourceName().replace("-", "_"), this.ae.getResourceID(), NOTIFICATIONCONTENT_TYPE.ALL_ATRRIBUTES.Value(), acpIds, true);
			ringtoneReqCntSubs = aeController.doCreateSubscription(ringtoneCnt.getUri(), this.aeId, 
					"sub-"+this.ae.getResourceName().replace("-", "_"), this.ae.getResourceID(), NOTIFICATIONCONTENT_TYPE.ALL_ATRRIBUTES.Value(), acpIds, true);
			...
			this.notiHandler.setResource(this.ae, locationReqCnt, locationCnt);
			this.notiHandler.setResource(ringtoneReqCnt, ringtoneCnt);
